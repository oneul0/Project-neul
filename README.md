...
| 영역 | 내용 |
|------|------|
| 제품 방향 정의 | "스트리머 본인이 편집 후보를 빠르게 찾는다"는 핵심 가치 설정 |
| 아키텍처 결정 | 마이크로서비스 분리 방식, Kafka 토픽 설계, 인증 구조 최종 결정 |
| 기술 스택 선택 | Spring WebFlux, pgvector, Ollama 도입 판단 |
| 검증 및 방향 수정 | AI가 제안한 구현을 직접 실행·테스트하고 문제를 피드백 |
| 보안 요구사항 정의 | 어떤 경로를 보호할지, 어느 수준의 방어가 필요한지 직접 판단 |

### AI 활용 방식

| 단계 | 활용 내용 |
|------|----------|
| **설계** | 아키텍처 트레이드오프 검토, ADR 초안 작성, 패키지 구조 제안 |
| **구현** | Spring Boot WebFlux 필터·서비스 레이어, Next.js API proxy, Kafka 파이프라인 코드 생성 |
| **보안 검토** | `OwnerIdentityResolver` 헤더 폴백 취약점 발견, `InternalAccessFilter` 설계 및 구현 |
| **디버깅** | SSE 메시지 유실(`Sinks.replay`), Kafka consumer group 꼬임, Flyway 마이그레이션 오류 원인 분석 |
| **리팩터링** | 서비스명 neul → gak 전체 이관, 패키지·환경변수·Redis 키 일괄 정리 |
| **문서화** | 트러블슈팅 가이드, 보안 강화 기록, 프로덕션 배포 체크리스트, 이 README 작성 |
| **코드 리뷰** | 변경 범위 영향 분석, 잠재 버그 지적, 개선 방향 제안 |

### AI 활용 역량 포인트

**지시의 정밀도** — "코드 짜줘"가 아니라 요구사항과 제약 조건을 명확히 전달했습니다.  
예: *"쿠키 검증 실패 시 헤더·쿼리 폴백을 완전히 제거하고 쿠키만 허용하도록 수정해줘. 이유는 X-Chzzk-Owner-Id 헤더로 타인 계정 위조가 가능하기 때문이야."*

**결과물 검증** — AI가 생성한 코드를 그대로 쓰지 않고 직접 실행·테스트 후 문제를 피드백해 반복 개선했습니다.

**범위 판단** — 무엇을 AI에 맡기고 무엇을 직접 결정할지 구분했습니다.  
구현 세부사항은 AI에게, 아키텍처 결정과 최종 승인은 개발자가 유지했습니다.

**컨텍스트 관리** — 프로젝트 전반의 흐름(ADR, 트러블슈팅 이력, 보안 모델)을 문서로 축적해 AI가 매 대화마다 일관된 판단을 내릴 수 있도록 했습니다.
