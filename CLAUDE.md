...
### 3-에이전트 자동 워크플로우 (agent_workflow)

새 기능 구현이나 버그 수정처럼 **영향 범위가 불명확한 작업**을 요청받으면,
EnterPlanMode 전에 아래 명령을 먼저 실행한다:

```bash
python scripts/run_workflow.py "작업 내용 설명"
```
...
**실행 조건 (아래 중 하나라도 해당하면 실행):**
- 새 클래스·서비스 추가
- 기존 서비스 메서드 시그니처 변경
- 여러 파일에 걸친 수정
- 처음 보는 모듈/패키지를 수정해야 하는 경우

**실행 조건 (해당하지 않으면 생략해도 됨):**
- 주석·문서 수정
- 로그 메시지 변경
- 명확한 단순 버그 수정 (한 파일, 몇 줄 이내)

Reviewer가 ✅ 승인하면 계획대로 구현을 진행한다.
Reviewer가 ❌이면 이유를 사용자에게 알리고 작업을 재정의한다.
