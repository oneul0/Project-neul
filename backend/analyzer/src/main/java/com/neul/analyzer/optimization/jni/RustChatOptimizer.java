 ...
 * <h3>현재 상태 (Stub)</h3>
 * <ul>
 * <li>네이티브 라이브러리({@code neul_optimizer.dll} / {@code libneul_optimizer.so}) 로드를
 * 시도합니다.</li>
 ...
 * {@code Java_com_neul_analyzer_optimization_jni_RustChatOptimizer_optimizeNative}
 * 시그니처 구현</li>
 * <li>빌드: {@code cargo build --release} → {@code neul_optimizer.dll / .so}
 * 생성</li>
 * <li>배포: {@code src/main/resources/native/} 경로에 라이브러리 위치</li>
 * <li>이 클래스의 {@link #optimize} 메서드에서 JSON 직렬화/역직렬화 구현 후 Fallback 제거</li>
 * </ol>
 *
 * @see com.neul.analyzer.optimization.ChatOptimizerConfig
 */
@Slf4j
public class RustChatOptimizer implements ChatOptimizer {

    /** 로드할 네이티브 라이브러리 이름 (OS별 접두사/확장자는 JVM이 자동 처리) */
    private static final String NATIVE_LIB_NAME = "neul_optimizer";
     ...
     * <pre>
     * #[no_mangle]
     * pub extern "system" fn Java_com_neul_analyzer_optimization_jni_RustChatOptimizer_optimizeNative(
